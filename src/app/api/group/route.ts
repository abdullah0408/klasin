...
/**
 * POST /api/course/material/group
 *
 * This endpoint creates a new folder (content group) within a course.
 ...
 *   - groupId: string (optional) — the ID of the parent group if this is a nested folder
 *
 * Response codes:
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { title, description, courseId, groupId } = body;
    ...
    //
    // Create the folder (content group)
    //
    const folder = await prisma.contantGroup.create({
      data: {
        title,
        description,
        courseId,
